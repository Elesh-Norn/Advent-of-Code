...
class Computer:
    ...
    def run(self, robotcode=None, manual=False)->(int, bool):
        instruction = deque()
        newline = False
        text = []
        while True:
            ...
            if outcode.opcode == 99:
                ...
                if outcode.opcode == 3:
                    if text:
                        ...
                        text = []
                        return "".join(value)
                    if not instruction and not newline:
                        if manual:
                            instruction = deque(input("Enter something:"))
                        else:
                            instruction = deque(robotcode)
                        newline = True

                    if not instruction and newline:
                        instruction.append("\n")
                        newline = False
                    ...
                    self.pointer = self.function_dico[outcode.opcode](outcode,
                                                                          self.intcodes,
                                                                          self.pointer,
                                                                          self.base, ord(instruction.popleft()))
